...
def harrison_ford
  ...
  Movie
    .select(:id, :title) #id and title
    .joins(:actors) #movies + actors  #actors table has a through to castings table
    .where(actors: {name: "Harrison Ford"}) #we get harrison's name in actors, and the through to casting
    .where("castings.ord != 1") #with access to through, we get castings.ord
